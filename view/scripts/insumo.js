...
function listar()
{
    tabla = $('#tbllistado').dataTable({
        ...
        buttons: [
                  {
                   ...
                   },
                    {
                        extend: 'print',
                        text: 'IMPRIMIR',
                        title: 'Insumos del Sistema  Clinica Génesis'
                    },
                    {
                        extend: 'pdf',
                        text: 'DESCARGAR PDF',
		...
		"ajax":
				{
					url: '../controller/insumo.php?op=listar',
					type : "get",
					dataType : "json",
					error: function(e){
...
function guardaryeditar(e)
{
    ...
    $.ajax({
        url: "../controller/insumo.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
        ...
        success: function (datos) {
            if (datos == 1) {
                ...
                Swal.fire({
                    position: 'top-end',
                    icon: 'error',
                    title: 'Insumo no se pudo actualizar',
                    showConfirmButton: false,
                    timer: 1500
                })
            }else if (datos == 2) {
                Swal.fire({
                    position: 'top-end',
                    icon: 'error',
                    title: 'No se pudo registrar el insumo',
                    showConfirmButton: false,
                    timer: 1500
                })
            }
...
{
	$.post("../controller/insumo.php?op=mostrar",{idinsumo : idinsumo}, function(data, status)
	{
...
function desactivar(idinsumo) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            $.post("../controller/insumo.php?op=desactivar", {
                idinsumo: idinsumo
            }, function (e) {
                tabla.ajax.reload();
            });
...
function activar(idinsumo) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.post("../controller/insumo.php?op=activar", {
                idinsumo: idinsumo
            }, function (e) {
                tabla.ajax.reload();
            });
